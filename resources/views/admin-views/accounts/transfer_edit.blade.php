...
@section('title', \App\CPU\translate('Transfer Edit'))
    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">{{ \App\CPU\translate('Transfer Edit') }}</li>
            </ol>
        </nav>
    </div>
    {{-- add --}}
    <form class="user" action="{{ route('admin.account.transfer_update') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="card">
            <div class="card-body">
                <div class="card-header">
                    <h5>{{ \App\CPU\translate('Edit Transfer') }}</h5>
                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="from_account" id="" class="form-control form-control-user" required>
                                ...
                                @foreach ($accounts as $row)
                                    <option value="{{ $row->id }}"
                                        @if($row->id==$transfer->from_account_id)
                                        {
                                            {{'selected'}}
                                        }
                                        @endif
                        ...
                        <div class="form-group">
                            <label for="exampleFirstName"
                                ...
                                <select name="to_account" id="" class="form-control form-control-user" required>
                                    ...
                                    @foreach ($accounts as $row)
                                        <option value="{{ $row->id }}"
                                            @if($row->id==$transfer->to_account_id)
                                            {
                                                {{'selected'}}
                                            }
                                            @endif
                    ...
                    <div class="col-lg-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control form-control-user" id="exampleFirstName"
                                name="amount" placeholder="{{ \App\CPU\translate('Ex: 4000.00') }}" value="{{$transfer->amount}}" required>
                        </div>
                    </div>
                </div>
                <div class="d-flex align-items-center justify-content-end gap-10">
                    <button type="submit" class="btn btn--primary btn-user"
                        id="apply">{{ \App\CPU\translate('Update Transfer') }}</button>
                </div>
            </div>
        </div>
    </form>

...
@section('content')
@php
    $courier_service=DB::table('courier_service')->get();
@endphp
        ...
        <div class="row gy-3" id="printableArea">
            <div class="col-lg-8 col-xl-9">
                ...
                <div class="card h-100">
                    ...
                    <div class="card-body">
                        <div class="d-flex flex-wrap gap-10 justify-content-between mb-4">
                            <div class="d-flex flex-column gap-10">
                                <h4 class="text-capitalize">{{\App\CPU\translate('Order_details')}} #{{$order['id']}}</h4>
                                <div class="">
  				 ...
  				 <a class="btn btn--primary px-4" target="_blank" title="{{\App\CPU\translate('Chalan')}}"
                                                href="{{route('admin.orders.generate-chalan',[$order['id']])}}">
                                                <i class="tio-print mr-1"></i> {{\App\CPU\translate('Print_Chalan')}}
                                            </a>
                                  <a class="btn btn--primary px-4" target="_blank" title="{{\App\CPU\translate('Chalan')}}"
                                    href="{{route('admin.orders.generate-mushok',[$order['id']])}}">
                                    <i class="tio-print mr-1"></i> {{\App\CPU\translate('Print_Mushok')}}
                                   </a>
                                </div>
                            ...
                            <table class="table fz-12 table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100">
                                ...
                                @foreach($order->details as $key=>$detail)
                                    @if($detail->product)
                                                ...
                                                <div class="media align-items-center gap-10">
                                                    ...
                                                    <div>
                                                       {{-- <h6 class="title-color">{{substr($detail->product['name'],0,30)}}{{strlen($detail->product['name'])>10?'...':''}}</h6> --}}
                                                        ...
                                                        <div><strong>{{\App\CPU\translate('Price')}} :</strong> {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(($detail['price']+$detail['discount'])))}}</div>
                                        ...
                                        @php($subtotal_val+=($detail['price']*$detail->qty))
                        ...
                        if ($order['extra_discount_type'] == 'percent') {
                            $ext_discount = $subtotal * ($order['extra_discount']/100);
                        } else {
                        ...
                        <div class="row justify-content-md-end mb-3">
                            <div class="col-md-9 col-lg-8">
                                <dl class="row gy-1 text-sm-right">
                                    <dt class="col-6">{{\App\CPU\translate('Subtotal')}}</dt>
                                    <dd class="col-6 title-color">
                                        <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($subtotal_val+$tax))}}</strong>
                                    </dd>
                                    <dt class="col-6">{{\App\CPU\translate('Delivery Fee')}}</dt>
                                    <dd class="col-6 title-color">
                                        <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order['shipping_cost']))}}</strong>
                                    </dd>
                                    
                                    @if($vatAmt > 0)
                                    <dt class="col-6">{{\App\CPU\translate('VAT')}}</dt>
                                    <dd class="col-6 title-color">
                                        <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($vatAmt))}}</strong>
                                    </dd>
                                    @endif
                                    ...
                                    <dt class="col-6">{{\App\CPU\translate('loyalty_discount')}}</dt>
                                    <dd class="col-6 title-color">
                                        - {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(round($order['loyalty_discount'],2)))}}
                                    </dd>
                                    @endif
                                    
                                    @if(isset($feeInfo['fee_amount']) && $feeInfo['fee_amount'] != '0')
                                    <dt class="col-6">{!! isset($feeInfo['fee_title']) ? $feeInfo['fee_title'] : ''  !!}</dt>
                                    <dd class="col-6 title-color">
                                         {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($feeAmt))}}
                                    </dd>
                                    @endif
                ...
                <div class="card">
                    <div class="card-body text-capitalize d-flex flex-column gap-4">
                        ...
                        <li class=" mt-2" id="by_third_party_delivery_service_info">
                            ...
                            <label class="font-weight-bold title-color fz-14" style="margin-bottom: -10px;">
                                Order Delivery
                            </label>
                            @if($order['order_status']=='confirmed' && $order['delivery_status']=='0')
                            <p class="btn btn-md btn-primary add_parcel" onclick="addParcel('third_party_delivery')">Add Parcel</p>
                            @endif
                        {{-- @endif --}}
                    </div>
                </div>
               ...
               {{-- mushok no --}}
                <div class="card">
                    ...
                    @if($order->customer)
                        <div class="card-body">
                            <h4 class="mb-4 d-flex gap-2">
                                {{\App\CPU\translate('Challan_number')}}
                            </h4>
                            <form action="{{route('admin.orders.challan_no_add')}}" method="POST">
                                @csrf
                                <div class="media flex-wrap gap-3">
                                    <div class="media flex-wrap gap-3">
                                        <div class="">
                                           <input type="text" name="challan_no" class="form-control" placeholder="challan number" value='{{isset($order->challan_no) ?$order->challan_no:''}}' required>
                                ...
                                <div>
                                    <button type="submit" class="btn btn-md btn-primary add_parcel">ADD Challan Number</button>
                                </div>
                            </form>
                    ...
                    @if($order->customer)
                        <div class="card-body">
                            ...
                            <div class="d-flex flex-column gap-2">
                                ...
                                <div>
                                    <span>{{\App\CPU\translate('Company Name')}}:</span>
                                    <strong>{{$billing ? $billing->company_name  : ''}}</strong>
                                </div>
                                <div>
                                    <span>{{\App\CPU\translate('BIN')}}:</span>
                                    <strong>{{isset($billing->company_bin) ? $billing->company_bin  : ''}}</strong>
                                </div>
                                <div>
                                    <span>{{\App\CPU\translate('PO')}}:</span>
                                    <strong>{{$order['purchase_order_no']}}</strong>
                                </div>
                                <div>
                                    <span>{{\App\CPU\translate('Contact')}}:</span>
                        ...
                        <div class="media">
                            ...
                            @else
                                @if(!empty($order->seller->shop))
                                    <div class="mr-3">
                                        <img class="avatar rounded avatar-70" onerror="this.src='https://6valley.6amtech.com/public/assets/front-end/img/image-place-holder.png'"
                                             src="{{!empty($order->seller->shop) ? asset('storage/app/public/seller/'.$order->seller->shop->image) : ''}}" alt="">
                                    </div>
                                    <div class="media-body d-flex flex-column gap-2">
                            ...
                            <form action="{{route('admin.orders.update-deliver-info')}}" method="POST">
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="" name="delivery_service_name" required>
                                            <option value="0">--select--</option>
                                            @foreach ($courier_service as $item)
                                                <option value="{{$item->name}}" 
                                                    <?php if ($item->name== $order['delivery_service_name']) {
                                                        echo 'selected';
                                                    } ?>
                                                    >{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                        {{-- <input class="form-control" type="text"  value="{{$order['delivery_service_name']}}" id="" required> --}}
                                    </div>
                                    <div class="form-group">
    ...
    <!--Show delivery info Modal -->
    <div class="modal" id="parcel_chose" role="dialog" tabindex="-1" >
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Parcel Create</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-12">
                            <form action="{{route('admin.parcel.store')}}" method="POST">
                                @csrf
                                <input type="hidden" name="order_id" value="{{$order['id']}}">
                                <input type="hidden" name="state" value="order_details">
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="">Post Code</label>
                                        <input class="form-control" type="text" name="post_code" value="{{$order['post_code']}}" id="" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="">Weight</label>
                                        <input class="form-control" type="text" name="weight" value="" id="">
                                    </div>
                                    <button class="btn btn-primary" type="submit">Create Parcel</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
@push('script_2')
    ...
    <script>
        ...
        function addParcel(val) {
            $('#parcel_chose').modal("show");
        }
    </script>
    <script>

        ...
        <div class="card row">
            <div class="col-md-12">
                <div class="card-body">
                    ...
                    <form class="product-form" action="{{route('admin.quotation.update',$quotation->id)}}" method="POST"
                                ...
                                <div class="row">
                                            ...
                                            <div class="input-group">
                                                <select required name="customer_id" id="customer_id" class="customer_select selectpicker form-control" data-live-search="true" title="Select customer..." style="width: 100px">
                                                    @foreach($lims_customer_list as $customer)
                                                        <option value="{{$customer->id}}" @if($customer->id == $customer_id) selected @endif>{{$customer->f_name . ' ( ' . $customer->phone . ' | ' . $customer->email  . ' )'}}</option>
                                                    @endforeach
                                                </select>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input type="text" name="subject" required class="form-control subject_input" placeholder="Write your subject line" value="{!!   ($quotation->subject) ? $quotation->subject : request()->get('subject_line') !!}">
                                                <input type="hidden" name="request_id"  value="{!!   request()->get('request_id') !!}">
                                            </div>
                                        </div>

                                    </div>
                                    <div class="pl-3" id="customer_info">
                                        <strong>Company Name:</strong><span id="quote_company_name">{{isset($contact->company)?$contact->company:''}}</span>
                                        <strong>Address:  </strong><span id="quote_address">{{isset($contact->address)?$contact->address:''}}</span>
                                        <strong>Email:  </strong><span id="quote_cust_email">{{isset($contact->email)?$contact->email:''}}</span>
                                        <strong>Phone:  </strong><span id="quote_cust_phone">{{isset($contact->phone)?$contact->phone:''}}</span>
                                        <button type="button" class="btn btn-link edit_cart_product" data-toggle="modal" data-target="#edit_qoute_address"><i class="tio-edit"></i>Edit</button>
                                    </div>
                                    <div class="col-md-12 mt-2">
                                        <label>{{\App\CPU\translate('file.Select Product')}}</label>
                                        <div class="search-box input-group">
                                            <button class="btn btn-secondary" style="pointer-events: none;"><i class="tio-barcode"></i></button>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="quotation_vat_status" id="quotation_vat_status" onchange="vat_status_change()">
                                                <option value="0" <?php if($quotation->is_vat=='0'){echo "selected";}?>>{{\App\CPU\translate('Without_Vat')}}</option>
                                                <option value="1" <?php if($quotation->is_vat=='1'){echo "selected";}?>>{{\App\CPU\translate('With_Vat')}}</option>
                                            </select>
                                        </div>
                                ...
                                <div class="row ">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <textarea rows="5" name="shipping_description" class="form-control">{!!$quotation->shipping_description!!}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>Shipping Cost</label>
                                            <input type="number" name="shipping_cost" class="form-control shipping_cost" value="{!!$quotation->shipping_cost!!}"/>
                                        </div>
                                    </div>
                                    <div class="col-md-3" style="display: none" id="shipping_vat_id">
                                        <div class="form-group">
                                            <label>Shipping Vat</label>
                                            <input type="text" name="shipping_vat" class="form-control shipping_vat" value="{!!$quotation->shipping_vat!!}"/>
                                        </div>
                                    </div>
                                    <div class="col-md-3" id="shipping_amount">
                                        <div class="form-group">
                                            <label>Shipping Amount</label>
                                            <input type="number" name="shipping_amount" class="form-control shipping_amt" value="{!!$quotation->shipping_amount!!}" readonly/>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <textarea rows="5" name="note" class="textarea editor-textarea" class="form-control">{!!$quotation->note!!}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        if(quotation_vat_status==1)
        {
            ...
            $('#shipping_amount').css('display','block');
        }
        else{
            $('#quotation_vat_id').css('display','none');
            $('#shipping_vat_id').css('display','none');
            $('#shipping_amount').css('display','none');
        }
        
        var quote_company_name=$('#quote_company_name').text();
        var quote_address=$('#quote_address').text();
        var quote_cust_email=$('#quote_cust_email').text();
        var quote_cust_phone=$('#quote_cust_phone').text();
        $('#edit_quote_company').val(quote_company_name);
        $('#edit_quote_company_address').val(quote_address);
        $('#edit_quote_phone').val(quote_cust_phone);
        $('#edit_quote_email').val(quote_cust_email);
    });   
...
var pos;
var cartData = '{!! json_encode($items); !!}';


$('[data-toggle="tooltip"]').tooltip();

	$.get('{!! route('admin.quotation.product.getProductData') !!}', function(data) {
	        lims_product_array = [];
	        product_code = data[0];
	        product_name = data[1];
	        product_qty = data[2];
    ...
    function removeProduct(key) {
        ...
        $.ajax({
            type: 'GET',
            url: '{!! route('admin.quotation.product.quotation_session_remove') !!}',
            data: {
                id: id
            },
...
function addNewProduct(data){
    $.ajax({
        type: 'GET',
        url: '{!! route('admin.quotation.product.quotation_session_insert') !!}',
        data: {
            data: data
        },
        success: function(data) {
...
function singleSubmit() {
    ...
    $.ajax({
        type: 'GET',
        url: '{!! route('admin.quotation.product.quotation_session_update') !!}',
        data: formData,
        success: function(data) {
            if(data.status == 'success') {
                $(".quotation_session").empty().html(data.view);
...
function calculateShippingCharge() {
    ...
    var shippingVat = $('.shipping_vat').val();

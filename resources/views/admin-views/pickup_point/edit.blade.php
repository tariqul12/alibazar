...
@section('title', \App\CPU\translate('Pick Up Point Edit'))
                ...
                <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <form class="g-3" action="{{route('admin.pick_up_points.store')}}" method="post">
                        ...
                        <div class="row align-items-center">
                            ...
                            <input type="text" class="form-control" id="pickname" placeholder="Enter your name" name="name" value="{{ $pickup_point->name }}">
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="pickphone" class="form-label">Phone</label>
                            <input type="tel" class="form-control" id="pickphone" placeholder="Enter your phone" name="phone" value="{{ $pickup_point->phone }}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row align-items-center">
                            ...
                            <textarea class="form-control" id="plocation" rows="3" name="address">{{ $pickup_point->address }}</textarea>
                                </div>
                                ...
                                <div class="col-lg-6">
                                    ...
                                    @php
                                        $id=$pickup_point->pick_up_status;
                                    @endphp
                                    <select class="form-control" name="pick_up_status" id="pstatus">
                                        <option value="0" class="d-none" selected>Select Any One</option>
                                        <option value="1" @if ($id==1) selected @endif>Published</option>
                                        <option value="2" @if ($id==2) selected @endif>Draft</option>
                                    </select>
                                    </div>
                                </div>
                                </div>
                        ...
                        <div class="d-flex align-items-center justify-content-end gap-10 m-3">
                            <button type="submit" class="btn btn--primary btn-user"
                                id="apply">{{ \App\CPU\translate('Update Pickup Point') }}</button>
                        </div>
                    </form>
                </div>
            </div>

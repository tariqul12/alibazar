...
<div class="navbar-tool dropdown {{Session::get('direction') === "rtl" ? 'mr-3' : 'ml-3'}}"
     style="margin-{{Session::get('direction') === "rtl" ? 'left' : 'right'}}: 0px">
    ...
    {{--@if($customer_id)--}}
     <a class="navbar-tool-icon-box dropdown-toggle" href="{{route('shop-cart')}}">
        ...
        <i class="navbar-tool-icon czi-cart"></i>Cart
    </a>
    {{--@else--}}
    ...
    {{--                    {{$cart->count()}}--}}
    {{--                </span>--}}
    {{--                <i class="navbar-tool-icon czi-cart"></i>Cart</a>--}}
    {{-- @endif --}}
        ...
        <div class="widget widget-cart px-3 pt-2 pb-3">
            @if($cart->count() > 0)
                        ...
                        <div class="widget-cart-item pb-2">
                            ...
                            <div class="media">
                                <a class="d-block {{Session::get('direction') === "rtl" ? 'ml-2' : 'mr-2'}}"
                                   href="{{route('frontend.product_details',$cartItem['slug'])}}">
                                    <img width="64"
                                ...
                                <div class="media-body">
                                    <h6 class="widget-product-title">
                                        <a href="{{route('frontend.product_details',$cartItem['slug'])}}">{{Str::limit($cartItem['name'],30)}}</a></h6>
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center py-3">
                    ...
                    {{-- @else
                        <a class="btn btn-outline-secondary btn-sm" 
                                    style="cursor: pointer" 
                                    data-toggle="modal" 
                                    data-target="#signInModal">{{\App\CPU\translate('Expand cart')}}
                ...
                <a class="btn btn--primary btn-sm btn-block" 
                                ...
                                data-toggle="modal" 
                                data-target="#signInModal">
                                <i class="czi-card {{Session::get('direction') === "rtl" ? 'ml-2' : 'mr-2'}} font-size-base align-middle"></i> {{\App\CPU\translate('Checkout')}}</a>
                @endif --}}
                
            @else
                <div class="widget-cart-item">
                    <h6 class="blank-cart text-center"><i

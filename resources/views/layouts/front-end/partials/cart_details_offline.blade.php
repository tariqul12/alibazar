                                    ...
                                    <div>
                                        ...
                                        <div class="spinbutton-wrapper">
                                            <div class="spinbutton" style="display: flex;">
                                                <button class="minus" type="button" onclick="offlineDecreseQtycart('{{ $minimum_order->minimum_order_qty }}','{{$cartItem['id']}}')">-</button>
                                                <input style="width: 75px;min-height: 26px;" class="val" type="number" name="quantity[{{ $cartItem['id'] }}]" id="offlineCartQuantity{{$cartItem['id']}}"
                                                         onchange="updateOfflineShopCartQuantity('{{ $minimum_order->minimum_order_qty }}', '{{$cartItem['id']}}')" min="{{ $minimum_order->minimum_order_qty ?? 1 }}" value="{{$cartItem['quantity']}}">
                                                <button class="plus" type="button" onclick="offlineincreaseQtycart('{{ $minimum_order->minimum_order_qty }}','{{$cartItem['id']}}')">+</button>
                                            </div>
                                        </div>
                                        </div>
...
<script>
     $.ajaxSetup({
       ...
       function updateOfflineShopCartQuantity(minimum_order_qty, key) {
            /* var quantity = $("#cartQuantity" + key).children("option:selected").val(); */
            var quantity = $("#offlineCartQuantity" + key).val();
            if (minimum_order_qty > quantity) {
                toastr.error('{{ \App\CPU\translate('minimum_order_quantity_cannot_be_less_than_') }}' +
                    minimum_order_qty);
                $("#offlineCartQuantity" + key).val(minimum_order_qty);
                return false;
            } 
        $.ajax({
                type: 'POST',
                url: "{{ route('cart.updateOfflineShopCartQuantity') }}",
                data: {
                    _token: '{{ csrf_token() }}',
                    key: key,
                    quantity: quantity
                ...
                if (response.status == 0) {
                    ...
                    $("#offlineCartQuantity" + key).val(response['qty']);
                    location.reload();
                } else {
                    updateNavCart();
        ...
        function offlineincreaseQtycart(minimum_order_qty,key){
            var quantity =parseInt($("#offlineCartQuantity"+key).val());
        ...
        function offlineDecreseQtycart(minimum_order_qty,key){
            var quantity =parseInt($("#offlineCartQuantity"+key).val());
            var new_quantity=quantity-1;
            if (minimum_order_qty > new_quantity) {
                toastr.error('{{ \App\CPU\translate('minimum_order_quantity_cannot_be_less_than_') }}' +
               ...
               var min=parseInt(minimum_order_qty)+1;
                $("#offlineCartQuantity" + key).val(min);
                return false;
            }
            $.ajax({
                type: 'POST',

...
Route::group(['namespace' => 'Web','middleware'=>['maintenance_mode']], function () {
    ...
    Route::get('blog', 'WebController@blog_page')->name('blog_page');
    Route::get('blog_view/{id}', 'WebController@blog_view')->name('blog_view');
    Route::get('quick-view', 'WebController@quick_view')->name('quick-view');
    Route::get('searched-products', 'WebController@searched_products')->name('searched-products');
    /*search all*/
    Route::get('search-all','WebController@searched_all_items')->name('searched-all-items');
    Route::get('search-single-type','WebController@searched_specific_item')->name('searched-single-type');
    // Compare
    ...
    Route::group(['middleware'=>['customer']], function () {
       // Route::get('checkout-details', 'WebController@checkout_details')->name('checkout-details');
        ...
        Route::get('eft-checkout-complete', 'WebController@eft_checkout_complete')->name('eft-checkout-complete')->middleware('customer');
    ...
    Route::get('quote-cart', 'WebController@quote_cart')->name('quote-cart');
    Route::get('quote_contact', 'WebController@quote_contact')->name('quote_contact');
    Route::get('quote_rfq_contact', 'WebController@quote_rfq_contact')->name('quote_rfq_contact');
    //wallet payment
    ...
    // po number add
    Route::post('order-po-number', 'UserProfileController@order_po_add')->name('order_po_add');
    Route::post('shipping-address-update', 'UserProfileController@shipping_address_update')->name('shipping_address_update');
    Route::post('shipping-address-update-store', 'UserProfileController@shipping_address_update_store')->name('shipping_address_update_store');
    // billing
    Route::post('billing-address-update', 'UserProfileController@billing_address_update')->name('billing_address_update');
    Route::post('billing-address-update-store', 'UserProfileController@billing_address_update_store')->name('billing_address_update_store');
    Route::post('courier-service', 'UserProfileController@order_wise_courier')->name('order_wise_courier');
    Route::group(['middleware'=>['customer']], function () {
        //profile Route
        ...
        Route::get('account-dashboard', 'UserProfileController@account_dashboard')->name('account-dashboard');
        Route::get('account-order', 'UserProfileController@account_oder')->name('account-oder');
        Route::get('account-quotation', 'UserProfileController@account_quotation')->name('account-quotation');
        Route::get('account-quotation/{id}', 'UserProfileController@quotation_show')->name('quotation-view');
        Route::get('quotation-download/{id}', 'UserProfileController@quotationDownload')->name('quotation-download');
...
Route::get('/product/{slug}',[\App\Http\Controllers\Frontend\ProductController::class,'details'])->name('frontend.product_details');
Route::get('/product_print/{slug}',[\App\Http\Controllers\Frontend\ProductController::class,'product_print'])->name('frontend.product_print');
...
Route::group(['prefix' => 'cart', 'as' => 'cart.', 'namespace' => 'Web'], function () {
    ...
    Route::post('review_remove', 'CartController@removeReviewFromCart')->name('review_remove');
    ...
    Route::post('updateShopCartQuantity', 'CartController@updateShopCartQuantity')->name('updateShopCartQuantity');
    Route::post('updateOfflineShopCartQuantity', 'CartController@updateOfflineShopCartQuantity')->name('updateOfflineShopCartQuantity');
    Route::post('updateReviewQuantity', 'CartController@updateReviewQuantity')->name('updateReviewQuantity');
    Route::post('updateQuoteQuantity', 'CartController@updateQuoteQuantity')->name('updateQuoteQuantity');
});
Route::group(['prefix' => 'quote', 'as' => 'quote.', 'namespace' => 'Web'], function () {
    Route::post('quote_add', 'CartController@addToQuote')->name('quote_add');
    Route::post('nav-quote-items', 'CartController@updateNavQuotation')->name('nav-quotation');
});

//Seller shop apply
Route::group(['prefix' => 'coupon', 'as' => 'coupon.', 'namespace' => 'Web'], function () {
    Route::post('apply', 'CouponController@apply')->name('apply');
    Route::post('coupon_remove', 'CouponController@coupon_remove')->name('coupon_remove');
});
//check done
...
/*start portwallet*/
Route::post('portwallet-pay','PortwalletPaymentController@indexV2');
Route::any('portwallet-redirect','PortwalletPaymentController@redirectV2')->name('portwallet-redirect');
Route::any('portwallet-ipn','PortwalletPaymentController@ipnV2')->name('portwallet-ipn');
...
Route::get("sms-test", function (){
    $smsModule = SMS_module::send("8801723264311", "123456");
    dd($smsModule);
});
Route::get('/{slug}', '\App\Http\Controllers\Web\WebController@show_custom_page')->name('custom-pages.show_custom_page');
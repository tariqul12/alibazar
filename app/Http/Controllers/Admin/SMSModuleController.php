...
class SMSModuleController extends Controller
{
    ...
    public function sms_update(Request $request, $module)
    {
        ...
        }elseif ($module == 'smsq_sms') {
            DB::table('business_settings')->updateOrInsert(['type' => 'smsq_sms'], [
                'type' => 'smsq_sms',
                'value' => json_encode([
                    'status' => $request['status'],
                    'api_key' => $request['api_key'],
                    ...
                    'otp_template' => $request['otp_template'],
                    'order_status_template' => $request['order_status_template'],
        ...
        if ($request['status'] == 1) {
            ...
            $config = Helpers::get_business_settings('smsq_sms');
            if (isset($config) && $module != 'smsq_sms') {
                DB::table('business_settings')->updateOrInsert(['type' => 'smsq_sms'], [
                    'type' => 'smsq_sms',
                    'value' => json_encode([
                        'status' => $request['status'],

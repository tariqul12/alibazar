...
class POSController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $data['net_unit_price'] = $net_unit_price;
        $data['discount'] = Helpers::get_product_discount($product, $price);
        $data['price'] = $price - $data['discount'];
    ...
    public function updatePrice(Request $request)
    {
        ...
        if($request->price>0){
            ...
            $keeper=[];
            $price = $request->price;
            \Log::info('key data:' . $request->key);
            \Log::info('cart info'. json_encode($cart));
            $variant_check = false;
            foreach ($cart as $item){
                \Log::info('item info'. json_encode($item));
                if (is_array($item)) {
                    if($price > 0)
                    {
                        if($item['id'] == $request->key) {
                            ...
                            $item['price'] = $price;
                            $item['discount'] = $product->unit_price - $price;
                            if($item['discount'] < 0){
                                $item['discount'] = 0;
                            }
    ...
    {
        $cart_id = session('current_user');
        if ($request->type == 'percent' && $request->tax_amount < 0) {
            Toastr::error(\App\CPU\translate('Extra_discount_can_not_be_less_than_0_percent'));
            return response()->json([
                'extra_discount' =>"amount_low",
                'view' => view('admin-views.pos._cart',compact('cart_id'))->render()
            ]);
        } elseif ($request->type == 'percent' && $request->tax_amount > 100) {
            Toastr::error(\App\CPU\translate('Extra_discount_can_not_be_more_than_100_percent'));
            return response()->json([
                'extra_discount' =>"amount_low",
                'view' => view('admin-views.pos._cart',compact('cart_id'))->render()
        ...
        if($cart!=null)
        {
            ...
            if ($request->type == 'percent') {
                $total_tax = ($total_product_price / 100) * $request->tax_amount;
            } else {
           ...
           // $total = $total_product_price - $product_discount + $product_tax - $coupon_discount - $ext_discount;
            ...
            {
                return response()->json([
                        'tax_status' =>"amount_low",
                        'view' => view('admin-views.pos._cart',compact('cart_id'))->render()
                    ]);
            }
            else{
                $cart['tax'] = $request->type == 'percent'?$request->tax_amount:BackEndHelper::currency_to_usd($request->tax_amount);
                $cart['tax_type'] = $request->type;
                session()->put($cart_id, $cart);
    ...
    public function update_vat(Request $request) {
        $cart_id = session('current_user');
        if ($request->type == 'percent' && $request->amount < 0) {
            Toastr::error("vat amount can not be less than zero");
            return response()->json([
                'vat' =>"amount_low",
                'view' => view('admin-views.pos._cart',compact('cart_id'))->render()
            ]);
        } elseif ($request->type == 'percent' && $request->amount > 100) {
            Toastr::error("Vat amount can not be more than 100");
            return response()->json([
                'vat' =>"amount_low",
                'view' => view('admin-views.pos._cart',compact('cart_id'))->render()
            ]);
        ...
        if($cart!=null)
        {
            ...
            if ($request->type == 'percent') {
                $total_vat = ($total_product_price / 100) * $request->amount;
            } else {
                $total_vat = $request->amount;
            }
            // $total = $total_product_price - $product_discount + $product_tax - $coupon_discount - $ext_discount;
            if($total_vat < 0)
            {
                ...
                $cart['vat'] = $request->type == 'percent'?$request->amount:BackEndHelper::currency_to_usd($request->amount);
                $cart['vat_type'] = $request->type;
                session()->put($cart_id, $cart);
    ...
    public function update_fee(Request $request) {
        $cart_id = session('current_user');
        if ($request->amount == 0) {
            Toastr::error("fee amount is zero");
            return response()->json([
                'vat' =>"amount_low",
                'view' => view('admin-views.pos._cart',compact('cart_id'))->render()
            ]);
        ...
        if($cart!=null)
        {
            $cart['fee_amount'] =$request->amount;
            $cart['fee_title'] = $request->title;
            session()->put($cart_id, $cart);
    ...
    public function update_pos_shipping_fee(Request $request) {
        $cart_id = session('current_user');
        if ($request->pos_shipping_amount == 0) {
            Toastr::error("Shipping fee amount is zero");
            return response()->json([
                'vat' =>"amount_low",
                'view' => view('admin-views.pos._cart',compact('cart_id'))->render()
            ]);
        ...
        if($cart!=null)
        {
            $cart['pos_shipping_amount'] =$request->pos_shipping_amount;
            session()->put($cart_id, $cart);
    ...
    public function place_order(Request $request)
    {
        ...
        $user_id = 0;
        $user_type = 'wc';
        $payment_status = isset($request['payment_status']) ? $request['payment_status'] : "paid";
        $delivery_status = isset($request['delivery_status']) ? $request['delivery_status'] : "delivered";
        if(Str::contains(session('current_user'), 'sc'))
        {
            $user_id = explode('-',session('current_user'))[1];
            $user_type = 'sc';
        ...
        if(isset($cart['fee_amount'])) {
            $feeInfo['fee_amount'] = $cart['fee_amount'];
            $feeInfo['fee_title'] = $cart['fee_title'];
            $total_price += $feeInfo['fee_amount'];
        } else {
            $feeInfo = [];
        }
        if(isset($cart['vat'])){
            $vat_amount = $cart['vat_type'] == 'percent' && $cart['vat'] > 0 ? (($product_price * $cart['vat']) / 100) : $cart['vat'];
        } else {
            $vat_amount = 0;
        }
        $trans_ref_no = $request->trans_reference_id;
        $discount_status_show = (isset($request->hide_product_discount) && $request->hide_product_discount == '1') ? 0 : 1;
        $shipping_method_id = isset($request->shipping_method_id) ? $request->shipping_method_id :  0;
        $same_as_shipping_address = $request->same_as_shipping_address;
        $same_as_billing_address = $request->same_as_billing_address;
        $billing_method_id = isset($request->billing_method_id) ? $request->billing_method_id : 0;
        $shippingInfo = ShippingAddress::where('id',$shipping_method_id)->first();
        if($shippingInfo) {
            $shippingData = json_encode($shippingInfo);
        } else {
            $shippingData = (isset($request->shipping) && is_array($request->shipping)) ? json_encode($request->shipping) : null;
        }
        $billingInfo = ShippingAddress::where('id',$billing_method_id)->first();
        if($billingInfo){
            $billingData = json_encode($billingInfo);
        } else {
            $billingData = (isset($request->billing) && is_array($request->billing)) ? json_encode($request->billing) : null;
        }
        /*if($same_as_shipping_address && $same_as_shipping_address == 'on'){
            $billingData = $shippingData;
        }*/

        if($same_as_billing_address && $same_as_billing_address == 'on'){
            $shippingData = $billingData;
        }

        $save_address_shipping = (isset($request->save_address_shipping) && $request->save_address_shipping == 'on') ? 1 : 0;
        $save_address_billing = (isset($request->save_address_billing) && $request->save_address_billing == 'on') ? 1 : 0;


        if($user_id == 0){
            $user_id = $this->customerCreation($request);
        } else {
            if($save_address_shipping && $shipping_method_id == 0){

                $shipping_method_id = $this->creationTypeMethod($request,"shipping",$user_id);
            }
            if($save_address_billing && $billing_method_id == 0) {
                $billing_method_id = $this->creationTypeMethod($request,"billing",$user_id);
            }
        }
        ...
        $or = [
            ...
            'shipping_cost'=>$cart['shipping_fee_amount']??0,
            'created_at' => now(),
            'updated_at' => now(),
            'order_by' => auth('admin')->user()->id,
            'order_via' => 'POS',
            'shipping_method_id' =>$shipping_method_id,
            'billing_address' =>$billing_method_id,
            'billing_address_data' => $billingData,
            'shipping_address_data' => $shippingData,
            'purchase_order_no' => $purchase_order_no,
            'vat_amount' => $vat_amount,
            'additional_fee' => json_encode($feeInfo),
            'trans_reference_id' => $trans_ref_no,
            'discount_status_show' => $discount_status_show,
        ];
   ...
   public function shipping_fee(Request $request) {
        ...
        if($cart!=null)
        {
            $cart['shipping_fee_amount'] =$request->shipping_fee_amount;
            session()->put($cart_id, $cart);
    ...
    public function customerCreation($request) {
        try{
            $shipping = isset($request->billing) ? $request->billing : [];
            $contact_person_name = isset($shipping['contact_person_name']) ? $shipping['contact_person_name'] : '';
            ...
            if($phone != '') {
                ...
                if($userInfo) {
                    return $userInfo->id;
                    //throw new \Exception("user found with this phone no");
                }
                $user = new User();
                $user->phone = $phone;
                $user->name = $contact_person_name;
                $user->f_name = $contact_person_name;
            ...
            \Log::error("customer error:" . $exception->getMessage());
            return 0;
        }
    }


    public function creationTypeMethod($request, $type,$user_id) {
        try{
            $items = [];
            if($type == "shipping") {
                $items = $request->shipping;
                $is_billing = 0;
            }
            if($type == "billing"){
            ...
            if(isset($items['contact_person_name']) && $items['contact_person_name'] != '' && isset($items['phone']) && $items['phone'] != '' && isset($items['address_type']) && $items['address_type'] != ''){
                $shippingAddress = new ShippingAddress();
                $shippingAddress->contact_person_name = $items['contact_person_name'];
                 $shippingAddress->company_bin = isset($items['company_bin'])?$items['company_bin']:'';
                $shippingAddress->phone = $items['phone'];
                $shippingAddress->address_type = $items['address_type'];
                $shippingAddress->customer_id = $user_id;
                $shippingAddress->address = (isset($items['address'])) ? $items['address'] : '';
                $shippingAddress->city = (isset($items['city'])) ? $items['city'] : '';
                $shippingAddress->zip = (isset($items['zip'])) ? $items['zip'] : '';
                $shippingAddress->company_name = (isset($items['company_name'])) ? $items['company_name'] : '';
                $shippingAddress->is_billing = $is_billing;
                $shippingAddress->save();
                return $shippingAddress->id;
            } else {
                throw new \Exception("contact person name missing");
            }


        }catch (\Exception $exception){
            \Log::error("type method error:". $exception->getMessage()." || ". $type);
            return 0;
        }
    }
    ...
    public function remove_discount(Request $request)
    {
        ...
        if(explode('-',session('current_user'))[0]=='wc')
        {
            ...
            $current =User::where('id',$user_id)->first();
            $current_customer = $current->f_name.' '.$current->l_name. ' (' .$current->phone.')';
            $shipping_addresses=ShippingAddress::where('customer_id',$current->id)->where('is_billing',0)->get();
            $billing_addresses=ShippingAddress::where('customer_id',$current->id)->where('is_billing',1)->get();
        }
        ...
        return response()->json([
            ...
            'current_customer'=>$current_customer,
            'view' => view('admin-views.pos._cart',compact('cart_id','shipping_addresses','billing_addresses'))->render()]);
    }
...
//zahed
    public function addInvoice(Request $request)
    {
        ...
        return view('admin-views.pos._add-invoice',compact('categories','cart_id','category','keyword','products'));
    }
//zahed

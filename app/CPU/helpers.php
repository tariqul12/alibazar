...
class Helpers
{
    ...
    {
        $data=DB::table('courier_service')->where('status',1)->get();
        return $data;
    }
    ...
    public function send_mail($email,$subject,$body,$file=null)
    {
        $data["email"] = $email;
        $data["title"] = $subject;
        $data["body"] = $body;
        if(!empty($file))
        {
            ...
            Mail::send('email.mail_body', $data, function ($message) use ($data) {
                $message->to($data["email"], $data["email"])
                    ->subject($data["title"]);
            });
        }
...
//            dd($str);
    ...
    {
        $categories = Category::select('id','name')->where('parent_id',0)->get()->toArray();
        $col    = 4;
        $chunks = array_chunk($categories, $col);
    ...
    public static function footer_brands()
    {
        $brands = Brand::active()->get()->take(24)->toArray();
        $col    = 4;
        $chunks = array_chunk($brands, $col);
    ...
    public static function footer_popular_search()
    {
        $popular_searches = ProSearch::orderBy('count','desc')->get()->take(24)->toArray();
        $col    = 4;
        $chunks = array_chunk($popular_searches, $col);
    ...
    public static function getFooter1Data()
    {
        $footerdata1=DB::table('footer')->where('widget_col',1)->get();
        return $footerdata1;
    }
    public static function getFooter2Data()
    {
        $footerdata2=DB::table('footer')->where('widget_col',2)->get();
        return $footerdata2;
    }
    
    public static function getImageMetaInfo($img)
    {
        $data=DB::table('uploads')->where('file_name',$img)->first();
        return $data;
    }
    ...
    {
        //$mpdf = new \Mpdf\Mpdf(['default_font' => 'FreeSerif', 'mode' => 'utf-8', 'tempDir' => storage_path('logs'), 'format' => [190, 250]]);
        $mpdf = new \Mpdf\Mpdf(['default_font' => 'poppins', 'mode' => 'utf-8', 'tempDir' => storage_path('logs'), 'format' => 'A4']);
    ...
    public static function quotation_mpdf($view, $file_prefix, $file_postfix)
    {
        //$mpdf = new \Mpdf\Mpdf(['default_font' => 'FreeSerif', 'mode' => 'utf-8', 'tempDir' => storage_path('logs'), 'format' => [190, 250]]);
        $mpdf = new \Mpdf\Mpdf(['default_font' => 'poppins','mode' => 'utf-8', 'tempDir' => storage_path('logs'), 'format' => 'A4']);
        ...
        $mpdf_view = $mpdf_view->render();
        $mpdf->WriteHTML($mpdf_view);
        $mpdf->Output('storage/app/public/quotation/'.$file_prefix . $file_postfix . '.pdf', 'F');
       
    }
   public static function invoice_mpdf($id)
    {
        ...
        $mpdf_view = view('web-views.invoice_new', compact('order', $order));
        $mpdf_view = $mpdf_view->render();
        $mpdf->WriteHTML($mpdf_view);
        $mpdf->Output('storage/app/public/order/'.'Order_invoice_' .$order->id . '.pdf', 'F');
...
if (!function_exists('callToApi')) {
    ...
    function callToApi($url, $data, $header = [], $method = 'POST')
    {
        try {
            ...
            if(isset($header['API-ACCESS-TOKEN']) && $header['API-ACCESS-TOKEN'] != '') {
                $headers[] = "API-ACCESS-TOKEN:".$header['API-ACCESS-TOKEN'];
            }
            ...
            \Log::info('API HEADER DATA:'.json_encode($headers));
            ...
            \Log::info('API RESPONSE DATA: '.json_encode($response));
            $err = curl_error($curl);
            ...
            if ($code == 200 & !($curlErrorNo)) {
               ...
               \Log::alert(json_encode($logMessage));

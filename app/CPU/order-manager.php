...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        #user loyalty point discount

        $user_loyalty = DB::table('users')->where('id', auth('customer')->id())->select('loyalty_point')->first();
        $user_point_amt = 0;
        if ($user_loyalty->loyalty_point > 0) {
            $user_point_amt = ($user_loyalty->loyalty_point / 2);
        }
        #courier add
        $courier_cost=CartManager::get_shipping_cost($data['cart_group_id']);
        $courier_amt=0;
        $courier_prod=DB::table('product_wise_courier')->where('product_id',$seller_data->product_id)->where('courier_id',$seller_data->courier_id)->first();
        $order_wise_cost=DB::table('cart_shippings')->where('cart_group_id',$data['cart_group_id'])->first();
   	 if (!empty($courier_prod)) {
            $courier_amt = ($courier_prod->amount * $seller_data->quantity);
            $courier_cost = ($courier_prod->amount * $seller_data->quantity) + $courier_cost;
        }
        DB::table('users')->where('id', auth('customer')->id())->update(["loyalty_point" => 0]);
        ...
        $or = [
            ...
            'loyalty_discount' => $user_point_amt,
            'discount_type' => $discount == 0 ? null : 'coupon_discount',
            'coupon_code' => $coupon_code,
            'order_amount' => CartManager::cart_grand_total($cart_group_id) - $discount - $user_point_amt,
            ...
            'shipping_cost' =>$courier_cost,
            "courier_id"=>$seller_data->courier_id,
            "courier_name"=>$seller_data->courier_name,
        ...
        $point_check=DB::table('loyalty_point_transactions')->where('user_id', auth('customer')->id())->orderBy('id','DESC')->first();
        if(!empty($point_check))
        {
            DB::table('loyalty_point_transactions')->where('id',$point_check->id)->update(['debit'=>$user_loyalty->loyalty_point,'credit'=>0,'balance'=>0]);
        }

        #user table update
        DB::table('users')->where('id', auth('customer')->id())->update(["loyalty_point" => $loyalty_point]);
        ...
        try {
            ...
            #order invoice mail send
            $datainfo['email'] = $user->email;
            $datainfo['title'] = 'Order Placed';
            $datainfo['id'] = $order_id;
            #pdf generate and storage 
            Helpers::invoice_mpdf($order_id);

            $files = [
                URL::to('/') . '/storage/app/public/order/Order_invoice_' . $order_id . '.pdf',
            ];
            #mail send
            Mail::send('email-templates.order-placed-v2', $datainfo, function ($message) use ($datainfo, $files) {
                $message->to($datainfo["email"], $datainfo["email"])
                    ->subject($datainfo["title"]);
